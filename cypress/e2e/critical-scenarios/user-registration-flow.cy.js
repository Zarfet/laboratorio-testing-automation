...
describe('Critical Scenario 3: User Registration Flow', { tags: ['@critical', '@registration'] }, () => {
  ...
  // INTEGRATION: End-to-End Registration with Custom Commands
  it('Should complete registration flow using custom SweetAlert commands', function() {
    cy.log('Test: Complete registration flow with enhanced popup handling');

    // Navigate to registration
    registrationPage.visit();

    // Use custom command for registration
    cy.registerUserWithPopupHandling(this.testUserData);

    // Use custom commands for popup verification
    cy.waitForSweetAlert('success');
    cy.verifySweetAlertTitle('Operación Exitosa');
    cy.verifySweetAlertMessage('Tu usuario ha sido creado correctamente');
    cy.clickSweetAlertConfirm('Ir al login');

    // Should be on login page
    cy.url().should('include', 'login');

    cy.log('Registration with custom commands completed successfully');
  });
